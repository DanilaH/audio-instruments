...
test("Surround has no horizontal overflow at 390x844 before or after capability negotiation", async ({ page }) => {
...
test("Surround keeps the primary 5.1 sequence action and Stop inside 1366x768", async ({ page }) => {
  await installExactFiveOneContext(page);
  await page.setViewportSize({ width: 1366, height: 768 });
  await page.goto("/surround-sound-test");
  await page.getByRole("button", { name: "Check surround support" }).click();

  await expectInsideViewport(page.getByRole("button", { name: "Test all 5.1 channels" }), 768);
