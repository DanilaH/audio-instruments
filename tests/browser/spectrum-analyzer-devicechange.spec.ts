...
test("devicechange never makes another input look active without explicit capture", async ({
  page,
}) => {
  await page.addInitScript(() => {
    ...
    Reflect.set(window, "__spectrumDevicechangeState", state);
    Reflect.set(window, "__removeActiveSpectrumDeviceFromEnumeration", () => {
      state.devices = ["mic-2"];
  ...
  await expect(page.locator("[data-spectrum-input]")).toHaveValue("mic-1");
  ...
  await page.evaluate(() => {
    const removeActive = Reflect.get(
      window,
      "__removeActiveSpectrumDeviceFromEnumeration",
    ) as () => void;
    removeActive();
  });
  ...
  await expect(page.locator("[data-spectrum-input]")).toHaveValue("mic-1");
  await expect(
    page.locator('[data-spectrum-input] option[value="mic-1"]'),
