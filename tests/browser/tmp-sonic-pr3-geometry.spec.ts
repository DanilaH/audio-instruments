...
for (const viewport of [
  ...
  test(`Tone geometry probe ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    const metrics = await page.evaluate(() => {
      const allSonic = [...document.querySelectorAll<HTMLElement>("[data-sonic-instrument]")];
    ...
    console.log(`PR3_TONE_GEOMETRY ${viewport.width}x${viewport.height}`, JSON.stringify(metrics));
  });
}
