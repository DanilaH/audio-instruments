...
test("Tone Generator exposes the safe idle baseline", async ({ page }) => {
  await expect(page.getByRole("heading", { name: "Tone Generator", level: 1 })).toBeVisible();
...
test("Tone Generator supports live controls and explicit Stop", async ({ page }) => {
  ...
  await expect(page.locator("[data-tone-frequency-readout]")).toContainText("Hz");
...
test("Tone primary interaction remains reachable in the 1366x768 first viewport", async ({
  ...
  const instrument = page.locator('[aria-label="Tone Generator controls and waveform"]');
  const playStop = page.locator("#tone-play-stop");
  ...
  expect((playBox?.y ?? 9999) + (playBox?.height ?? 0)).toBeLessThanOrEqual(768);
});
